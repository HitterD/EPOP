...
import { ProjectTaskEvent, Task } from '@/types'
    ...
    onEvent: useCallback(
      (event: any) => {
        ...
        if (taskEvent.projectId === projectId && taskEvent.patch) {
          ...
          queryClient.setQueryData(['project-buckets', projectId], (oldData: any) => {
            if (!oldData || !Array.isArray(oldData)) return oldData

            return oldData.map((bucket: any) => ({
              ...bucket,
              tasks: bucket.tasks
...
export function useOptimisticTaskUpdate(projectId: string) {
  ...
  const updateTaskOptimistically = useCallback(
    (taskId: string, updates: Partial<Task>) => {
      const previousData = queryClient.getQueryData(['project-tasks', projectId])
